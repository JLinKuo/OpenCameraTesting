package com.example.opencameratesting.opencamera.UI;
...
public class DrawPreview {
	...
	private CameraInterface.PhotoMode photoMode;
...
//
//			KeyguardManager keyguard_manager = (KeyguardManager)main_activity.requireActivity().getSystemService(Context.KEYGUARD_SERVICE);
...
//	        //int height = applicationInterface.drawTextWithBackground(canvas, p, current_time_string, Color.WHITE, Color.BLACK, location_x, location_y, CameraInterface.Alignment.ALIGNMENT_TOP);
...
//			}
//	        int height = applicationInterface.drawTextWithBackground(canvas, p, current_time_string, Color.WHITE, Color.BLACK, location_x, location_y, CameraInterface.Alignment.ALIGNMENT_TOP, null, true, text_bounds_time);
//			height += gap_y;
//			if( ui_rotation == 90 ) {
//				location_y -= height;
//			}
...
////				//int height = applicationInterface.drawTextWithBackground(canvas, p, free_memory_gb_string, Color.WHITE, Color.BLACK, location_x, location_y, CameraInterface.Alignment.ALIGNMENT_TOP);
...
////				}
////				int height = applicationInterface.drawTextWithBackground(canvas, p, free_memory_gb_string, Color.WHITE, Color.BLACK, location_x, location_y, CameraInterface.Alignment.ALIGNMENT_TOP, null, true, text_bounds_free_memory);
////				height += gap_y;
////				if( ui_rotation == 90 ) {
////					location_y -= height;
////				}
...
//						CameraInterface.Alignment.ALIGNMENT_BOTTOM, null, true);
//		}
...
//				int height = applicationInterface.drawTextWithBackground(canvas, p, iso_exposure_string, text_color, Color.BLACK, location_x, location_y, CameraInterface.Alignment.ALIGNMENT_TOP, ybounds_text, true);
//				height += gap_y;
...
//
//			if( (
//					photoMode == CameraInterface.PhotoMode.DRO ||
//					photoMode == CameraInterface.PhotoMode.HDR ||
//					photoMode == CameraInterface.PhotoMode.ExpoBracketing ||
//					photoMode == CameraInterface.PhotoMode.FocusBracketing ||
//					photoMode == CameraInterface.PhotoMode.FastBurst ||
//					photoMode == CameraInterface.PhotoMode.NoiseReduction
//					) &&
...
//				canvas.drawRect(icon_dest, p);
//				p.setAlpha(255);
//				Bitmap bitmap = photoMode == CameraInterface.PhotoMode.DRO ? dro_bitmap :
//						photoMode == CameraInterface.PhotoMode.HDR ? hdr_bitmap :
//						photoMode == CameraInterface.PhotoMode.ExpoBracketing ? expo_bitmap :
//						photoMode == CameraInterface.PhotoMode.FocusBracketing ? focus_bracket_bitmap :
//						photoMode == CameraInterface.PhotoMode.FastBurst ? burst_bitmap :
//						photoMode == CameraInterface.PhotoMode.NoiseReduction ? nr_bitmap :
//								null;
//				if( bitmap != null ) {
...
//				}
//				//applicationInterface.drawTextWithBackground(canvas, p, angle_string, color, Color.BLACK, canvas.getWidth() / 2 + pixels_offset_x, text_base_y, CameraInterface.Alignment.ALIGNMENT_BOTTOM, ybounds_text, true);
...
//				}
//				applicationInterface.drawTextWithBackground(canvas, p, angle_string, color, Color.BLACK, canvas.getWidth() / 2 + pixels_offset_x, text_base_y, CameraInterface.Alignment.ALIGNMENT_BOTTOM, null, true, Math.abs(cached_angle) < 10.0 ? text_bounds_angle_single : text_bounds_angle_double);
...
//				applicationInterface.drawTextWithBackground(canvas, p, string, color, Color.BLACK, canvas.getWidth() / 2 + pixels_offset_x, text_base_y, CameraInterface.Alignment.ALIGNMENT_BOTTOM, ybounds_text, true);
//			}
...
//					p.setTextSize(14 * scale + 0.5f); // convert dps to pixels
//					p.setTextAlign(Paint.Align.CENTER);
//					applicationInterface.drawTextWithBackground(canvas, p, getContext().getResources().getString(R.string.zoom) + ": " + zoom_ratio +"x", Color.WHITE, Color.BLACK, canvas.getWidth() / 2, text_base_y - text_y, CameraInterface.Alignment.ALIGNMENT_BOTTOM, ybounds_text, true);
//				}
//			}
//
//		}
			...
			if( has_pitch_angle && show_pitch_lines_pref ) {
				...
				for(int latitude_angle=-90;latitude_angle<=90;latitude_angle+=angle_step) {
					...
					if( Math.abs(this_angle) < 90.0 ) {
						...
						applicationInterface.drawTextWithBackground(canvas, p, "" + latitude_angle + "\u00B0", p.getColor(), Color.BLACK, (int)(cx + pitch_radius + 4*hthickness), (int)(cy + pitch_distance - 2*hthickness), CameraInterface.Alignment.ALIGNMENT_CENTRE);
					}
				}
			}
			if( has_geo_direction && has_pitch_angle && show_geo_direction_lines_pref ) {
				...
				for(int longitude_angle=0;longitude_angle<360;longitude_angle+=angle_step) {
					...
					if( Math.abs(this_angle) < 90.0 ) {
						...
						applicationInterface.drawTextWithBackground(canvas, p, "" + longitude_angle + "\u00B0", p.getColor(), Color.BLACK, (int)(cx + geo_distance), (int)(cy - geo_radius - 4*hthickness), CameraInterface.Alignment.ALIGNMENT_BOTTOM);
					}
				}
			}
		...
		if( enable_gyro_target_spot ) {
			...
			if( gyroSensor.isRecording() ) {
				...
				if( gyroSensor.isUpright() != 0 && Math.abs(angle_x) <= 20.0f*0.0174532925199f ) {
					//applicationInterface.drawTextWithBackground(canvas, p, "not upright", Color.WHITE, Color.BLACK, canvas.getWidth()/2, canvas.getHeight()/2, CameraInterface.Alignment.ALIGNMENT_CENTRE, null, true);
					canvas.save();
					canvas.rotate(ui_rotation, canvas.getWidth()/2.0f, canvas.getHeight()/2.0f);
